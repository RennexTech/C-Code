...
// Windows-specific header for _mkdir
#ifdef _WIN32
#include <direct.h>
#else
// Unix-like header for mkdir
#include <sys/stat.h>
#endif

// Password length for demonstration.
...
// Mocks unrar extraction, simulating success if password matches.
int mock_unrar_extract(const char* file_name, const char* path, const char* password, const char* dest_dir, const char* target_password) {
...
// Fills buffer with random printable ASCII characters.
void generate_random_password(char* buffer, int length) {
...
int main() {
    ...
    const char* target_password = "pass1"; // Hardcoded target password
    ...
    long long pswd_attempts = 0; // Use long long for large number of attempts
    ...
    while (1) {
        ...
        // Check password using the mock function
